...
<script>
        var markers;
        var map = L.map('map').setView([24.1690882, 120.6120678], 7);
    $(document).ready(function() {
        $.ajax({
            type: "get",
            url: "/admin/member/getLocationNum",
            success: function(locat) {
                ...
                locat.forEach(function(data) {
                    ...
                    markers = new L.marker([data.locationY, data.locationX]).addTo(map)
                        .bindPopup(data.location + " " + data.count + "人")
                        .openPopup();
                });
            }
        });
